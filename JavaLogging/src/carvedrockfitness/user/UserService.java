...
public class UserService {
    ...
    public boolean addUser(User user) {
        if (user.getDateCreated().isAfter(LocalDateTime.now())) {
            LOGGER.log(Level.WARNING, "Trying to create a user with a creation date that's in the future. User details: " + user); // 2
            try {
        ...
        LOGGER.log(Level.INFO, "Adding user, with user details: " + user); // 2
