...
public class OrderService {
	...
	public List<Order> getAllOrdersByUser(User user) {
		if (UserRepository.getDummyDataList().contains(user)) {
			...
			LOGGER.log(Level.WARNING, "The user was not found" + user); // 3
			try {
	...
	public boolean addOrder(Order order) {
		if (order.getOrderDateTime().isAfter(LocalDateTime.now())) {
			try {
				LOGGER.log(Level.WARNING, "Trying to place an order in the future, order details: " + order); // 2
		...
		if (order.getProducts().size() < 1) {
			try {
				LOGGER.log(Level.WARNING, "Trying to place an order with no products, order details: " + order); // 2
		...
		if (order.getUser().getUserStatus() == UserStatus.BLOCKED) {
			try {
				LOGGER.log(Level.WARNING, "Trying to place an order by a blocked user, order details: " + order); // 2
			...
			try {
				LOGGER.log(Level.WARNING, "Trying to place an order by a pending user, order details: " + order); // 2
		...
		LOGGER.log(Level.INFO, "Placing an order, order details: " + order); // 2
